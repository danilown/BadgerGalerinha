...
try:
    open("badge_data/badge-image.bin", "rb").readinto(BADGE_IMAGE)
except OSError:
    try:
        import badge_image
...
def draw_badge():
    ...
    display.thickness(2)
    display.text(detail1_text, 5, HEIGHT - ((DETAILS_HEIGHT * 3) // 2), DETAILS_TEXT_SIZE)
...
# Open the badge file
try:
    badge = open("badge_data/badge.txt", "r")
except OSError:
    with open("badge_data/badge.txt", "w") as f:
